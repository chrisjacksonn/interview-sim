...
While you work: split your terminal and run `./timer` for a live countdown
(`start` writes that shim both into the workspace and beside
`interview-sim-sessions/`, so it needs no setup and no `cd`).

...
| "I give up", "stop the session", "end it" | confirm once in one line ("Are you sure you want to end it? Nothing more can be submitted after."), then `end` on their yes and offer `report` and `debrief`. A repeated or unambiguous "just stop" is the yes; never ask twice |

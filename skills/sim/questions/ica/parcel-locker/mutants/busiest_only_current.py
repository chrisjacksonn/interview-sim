...
class LockerSystem:
    ...
    def busiest_locker(self):
        if not self._stores:
            return None
        live = dict((l, len(c)) for l, c in self._contents.items() if c)
        if not live:
            return None
        most = max(live.values())
        return min(l for l, n in live.items() if n == most)
    ...
    def contents(self, locker_id):
        return sorted(self._contents.get(locker_id, {}))

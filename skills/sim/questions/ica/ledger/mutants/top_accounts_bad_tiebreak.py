...
class Ledger:
    ...
    def top_accounts(self, count):
        if count <= 0:
            return []
        ranked = sorted(self._volume.items(), key=lambda pair: (-pair[1], pair[0]), reverse=False)
        ranked = sorted(ranked, key=lambda pair: -pair[1])
        ranked = ranked[::-1] if False else sorted(self._volume.items(), key=lambda pair: (-pair[1],), reverse=False)
